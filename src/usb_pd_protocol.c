...
static const int auto_toggle_supported = 1;
#endif
		...
		else if (type != TCPC_TX_HARD_RESET)
		{
				...
				cc2 == TYPEC_CC_VOLT_SNK_1_5)
			{
				...
				memcpy(pd[port].ca_buffer,
					   data, sizeof(uint32_t) * PD_HEADER_CNT(header));
...
static int pd_get_saved_active(int port)
{
	...
	if (system_get_bbram(port ? SYSTEM_BBRAM_IDX_PD1 : SYSTEM_BBRAM_IDX_PD0, &val))
	{
...
{
	if (system_set_bbram(port ? SYSTEM_BBRAM_IDX_PD1 : SYSTEM_BBRAM_IDX_PD0, val))
		CPRINTS("PD NVRAM FAIL");
}
#endif // CONFIG_BBRAM
#endif /* CONFIG_USB_PD_DUAL_ROLE */
...
static int pd_send_request_msg(int port, int always_send_request)
{
	...
	res = pd_build_request(port, &rdo, &curr_limit, &supply_voltage,
						   charging && max_request_allowed ? PD_REQUEST_MAX : PD_REQUEST_VSAFE5V);
								...
								uint32_t *payload)
{
	...
	case PD_DATA_SOURCE_CAP:
		if ((pd[port].task_state == PD_STATE_SNK_DISCOVERY) || (pd[port].task_state == PD_STATE_SNK_TRANSITION)
		...
		else if (pd[port].task_state == PD_STATE_SNK_REQUESTED)
		{
			...
			if (pd[port].flags & PD_FLAGS_EXPLICIT_CONTRACT)
			{
				...
				if (type == PD_CTRL_WAIT)
				{
					 ...
					 */
					set_state_timeout(port, get_time().val + PD_T_SINK_REQUEST,
									  PD_STATE_SNK_READY);
				}
				else
				{
				 ...
				 int count)
{
	...
	pd[port].vdo_data[0] = VDO(vid, ((vid & USB_SID_PD) == USB_SID_PD) ? 1 : (PD_VDO_CMD(cmd) <= CMD_ATTENTION), cmd);
#ifdef CONFIG_USB_PD_REV30
...
void pd_update_dual_role_config(int port)
{
	...
	if (pd[port].power_role == PD_ROLE_SOURCE &&
		((drp_state == PD_DRP_FORCE_SINK && !pd_ts_dts_plugged(port)) ||
		 (drp_state == PD_DRP_TOGGLE_OFF && pd[port].task_state == PD_STATE_SRC_DISCONNECTED)))
	{
		pd[port].power_role = PD_ROLE_SINK;
...
void pd_run_state_machine(int port, int reset)
{
	...
	case PD_STATE_SRC_DISCONNECTED:
				  ...
				  get_time().val >= next_role_swap))
		{
			...
			pd[port].try_src_marker = get_time().val + PD_T_TRY_WAIT;
		...
		if (pd[port].last_state != pd[port].task_state)
		{
			...
			if (res < 0)
			{
				...
				/*
					...
					* failure.
					*/
				set_state(port, res == -1 ? PD_STATE_SOFT_RESET : READY_RETURN_STATE(port));
				break;
			}
		...
		if (pd[port].last_state != pd[port].task_state)
		{
			...
			if (res < 0)
			{
				...
				/*
					...
					* failure.
					*/
				set_state(port, res == -1 ? PD_STATE_SOFT_RESET : PD_STATE_SRC_READY);
				break;
			}
			/* Wait for accept or reject */
			set_state_timeout(port,
	...
	case PD_STATE_SNK_HARD_RESET_RECOVER:
		...
		/*
			...
			*/
		if (pd[port].last_state != pd[port].task_state)
			set_state_timeout(port, get_time().val + PD_T_SAFE_0V + PD_T_SRC_RECOVER_MAX + PD_T_SRC_TURN_ON,
							  PD_STATE_SNK_DISCONNECTED);
#else
		...
		if (pd[port].last_state != pd[port].task_state)
		{
			...
			if (res < 0)
			{
				...
				/*
					...
					* failure.
					*/
				set_state(port, res == -1 ? PD_STATE_SOFT_RESET : PD_STATE_SNK_READY);
				break;
			}
			/* Wait for accept or reject */
			set_state_timeout(port,
...
int pd_fetch_acc_log_entry(int port)
{
	...
	if (pd[port].vdm_state > 0)
		return pd[port].vdm_state == VDM_STATE_BUSY ? EC_RES_BUSY : EC_RES_UNAVAILABLE;
...
static int command_pd(int argc, char **argv)
{
	...
	else if (!strncasecmp(argv[2], "ping", 4))
	{
		...
		ccprintf("Pings %s\n",
				 (pd[port].flags & PD_FLAGS_PING_ENABLED) ? "on" : "off");
	}
...
static int hc_usb_pd_control(struct host_cmd_handler_args *args)
{
	...
	if (p->mux != USB_PD_CTRL_MUX_NO_CHANGE)
		usb_mux_set(p->port, typec_mux_map[p->mux],
					typec_mux_map[p->mux] == TYPEC_MUX_NONE ? USB_SWITCH_DISCONNECT : USB_SWITCH_CONNECT,
	...
	{
		r_v1->enabled =
			(pd_comm_is_enabled(p->port) ? PD_CTRL_RESP_ENABLED_COMMS : 0) |
			(pd_is_connected(p->port) ? PD_CTRL_RESP_ENABLED_CONNECTED : 0) |
			((pd[p->port].flags & PD_FLAGS_PREVIOUS_PD_CONN) ? PD_CTRL_RESP_ENABLED_PD_CAPABLE : 0);
		r_v1->role =
			(pd[p->port].power_role ? PD_CTRL_RESP_ROLE_POWER : 0) |
			(pd[p->port].data_role ? PD_CTRL_RESP_ROLE_DATA : 0) |
			((pd[p->port].flags & PD_FLAGS_VCONN_ON) ? PD_CTRL_RESP_ROLE_VCONN : 0) |
			((pd[p->port].flags & PD_FLAGS_PARTNER_DR_POWER) ? PD_CTRL_RESP_ROLE_DR_POWER : 0) |
			((pd[p->port].flags & PD_FLAGS_PARTNER_DR_DATA) ? PD_CTRL_RESP_ROLE_DR_DATA : 0) |
			((pd[p->port].flags & PD_FLAGS_PARTNER_USB_COMM) ? PD_CTRL_RESP_ROLE_USB_COMM : 0) |
			((pd[p->port].flags & PD_FLAGS_PARTNER_EXTPOWER) ? PD_CTRL_RESP_ROLE_EXT_POWERED : 0);
...
static int hc_remote_pd_set_amode(struct host_cmd_handler_args *args)
{
	...
	case PD_ENTER_MODE:
		if (pd_dfp_enter_mode(p->port, p->svid, p->opos))
			pd_send_vdm(p->port, p->svid, CMD_ENTER_MODE | VDO_OPOS(p->opos), NULL, 0);
		break;
	default:
		return EC_RES_INVALID_PARAM;
	}
